...
$$\nabla\phi\cdotn_{\partial\Omega} = 0$$
Implimenting for a rectangular box:
...
(2) Dirichlet Boundary Conditions: Here, all $\phi_{i,j} = 0$ at all the boundary points
...
#####With $g(\phi)$:  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;We further modify our code to impliment $g(\phi)$. To benchmark our results with Nestlet$^1$, we simulate the following cases:

(1) $\epsilon = 1 and 10$ while keeping m=0: These conditions imply that as the driving force is zero, interface should not move, however two different values of $\epsilon$ shows the difference in interfacial thickness. These results are presented in the figures below. Further the animations are contained in the files named as fem_eps_1m_0.mp4 and fem_eps_10m_0.mp4 in the Dropbox folder.

(2) $m = 1 and -1$ while keeping $\epsilon = 1$: These conditions should make interface move in the opposite directions with respect to each other. For $m = 1$, interface will move downwards. In other words, $m = 1$ will make solid phase ($\phi = 1$) grow resulting in solidification while $m = -1$ will result in liquification where liquid phase will consume solid phase. These results presented in the figures below, compare excellently with Nestler.$^1$ Further the animations are contained in the files named as fem_eps_1m_1.mp4 and fem_eps_1m_-1.mp4 in the Dropbox folder.
 
(3) Simulating Random Microstructure: Here, we populate the liquid melt ($\phi = 0$ matrix) with multiple solid phase seeds (zones of $\phi = 0$). Periodic boundary conditions are applied to the simulation box. Values of m and $\epsilon$ are 5 and 1 respectively. Animation of this simulation is in the Dropbox folder as random_microstructure.mp4. As seen from this animations, we find that the problem behaves as heat diffusion with increasing amount liquid, which eventially converts to solid (\phi = 1). This situation seems unrealistic as our differential equation does not account for temperature effect. Hence, with control parameter m = 5, solid phase should strictly grow, while we do not observe the same. Further analysis and comparison with other methods are required to resolve this problem.
